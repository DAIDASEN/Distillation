...
export WANDB_DIR=${WANDB_DIR:-"simplerl-grpo-distill-qwen3-1.7b-${NOW}"}
export WANDB_PROJECT=${WANDB_PROJECT:-${WANDB_DIR}}
export WANDB_EXP=${WANDB_EXP:-"distill-${NOW}"}
...
DISTILL_ASYNC=${DISTILL_ASYNC:-1}   # 0 = sync KD, 1 = one_step_off KD
DISTILL_COEF=${DISTILL_COEF:-0.05}
DISTILL_TOPK=${DISTILL_TOPK:-50}
TEACHER_WORKERS=${TEACHER_WORKERS:-1}
...
set -x
python3 -m verl.experimental.simple_rl_distill.main_ppo \
  ...
  actor_rollout_ref.actor.distill.enabled=True \
  actor_rollout_ref.actor.distill.async_enabled="${DISTILL_ASYNC}" \
  actor_rollout_ref.actor.distill.coef="${DISTILL_COEF}" \
  actor_rollout_ref.actor.distill.topk="${DISTILL_TOPK}" \
  actor_rollout_ref.actor.distill.teacher_host="${TEACHER_HOST}" \
  actor_rollout_ref.actor.distill.teacher_port="${TEACHER_PORT}" \
  actor_rollout_ref.actor.distill.n_server_workers="${TEACHER_WORKERS}" \
  actor_rollout_ref.actor.distill.teacher_max_tokens=1 \
  actor_rollout_ref.actor.distill.teacher_only_response=False \
